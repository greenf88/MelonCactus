...
const limits = {
  ...
  outcome: 2_000, target: 1_000, timeframe: 100, report: 160, deliveryPriority: 32, serviceFocus: 80,
...
const singleLine = ["name", "email", "company", "role", "target", "timeframe", "report", "deliveryPriority", "serviceFocus"] as const;
...
export function validateEnquiry(input: unknown): ValidationResult {
  ...
  if (values.serviceFocus && !getFocusedService(values.serviceFocus)) return { ok: false, reason: "invalid" };
...
export function composeEnquiry(enquiry: Enquiry, receivedAt: Date) {
  ...
  const service = enquiry.serviceFocus ? getFocusedService(enquiry.serviceFocus) : undefined;
  if (enquiry.serviceFocus && !service) throw new Error("Invalid service focus");
  const fields = [
    ...
    ["Service focus", service?.title ?? "Not specified"],
